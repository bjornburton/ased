...
\M{7}The prescaler is set to clk/16484 at \PB{$\X26:Initialize the no-wave
timer\X$}.
...
the \PB{\.{"\\\_delay\\\_us()"}} function here \PB{$\X29:Hold-off all
interrupts\X$}.
...
\&{int} \\{main}(\&{void})\1\1 $\{$ \X24:Initialize pin outputs and inputs\X\par
\fi
...
\Y\B\X26:Initialize the no-wave timer\X\par
\fi
...
\Y\B\X27:Initialize the wave detection\X\par
\fi
...
\X29:Hold-off all interrupts\X $\}{}$\SHC{ end for }\6
\&{return} \T{0};\SHC{ it's the right thing to do! }\6
$\}{}$\SHC{ end main }\par
\fi

\N{1}{23}These are the supporting routines and configuration blocks.

\fi

\M{24}\B\X24:Initialize pin outputs and inputs\X${}\E{}$\6
...
\Y\B\4\X26:Initialize the no-wave timer\X${}\E{}$\6
...
\Y\B\4\X27:Initialize the wave detection\X${}\E{}$\6
...
\M{29}\B\X29:Hold-off all interrupts\X${}\E{}$\6
