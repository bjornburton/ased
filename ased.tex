...
\M{3}\PB{\.{"F\_CPU"}} is used only to convey the Trinket clock to delay.h.
\Y\B\4\D$\.{F\_CPU}$ \5
\T{8000000\$U\$L}\par
\fi
...
\M{6}\PB{\.{"WAVETHRESHOLD"}} is maybe about 250 ms. Don't take too long or
timer will overflow.
...
\M{7}The prescaler is set to clk/16484 at \PB{$\X25:Initialize the no-wave
timer\X$}.
\PB{\.{"TIMESTART"}} is the timer preset so that overflow happens in about
500~ms.
...
\M{8} This is the hold-off time in us for wave detection. This value is used by
the \PB{\.{"\\\_delay\\\_us()"}} function here \PB{$\X28:Hold-off all
interrupts\X$}.
...
\&{int} \\{main}(\&{void})\1\1 $\{$ \X23:Initialize pin outputs and inputs\X\par
\fi
...
\Y\B\X25:Initialize the no-wave timer\X\par
\fi
...
\Y\B\X26:Initialize the wave detection\X\par
\fi
...
\Y\B\X27:Configure to wake upon interrupt\X\par
\fi
...
\X28:Hold-off all interrupts\X $\}{}$\SHC{ end for }\6
\&{return} \T{0};\SHC{ it's the right thing to do! }\6
$\}{}$\SHC{ end main }\par
\fi

\M{23}\B\X23:Initialize pin outputs and inputs\X${}\E{}$\6
...
\Y\B\4\X25:Initialize the no-wave timer\X${}\E{}$\6
...
\Y\B\4\X26:Initialize the wave detection\X${}\E{}$\6
...
\Y\B\4\X27:Configure to wake upon interrupt\X${}\E{}$\6
${}\{{}$\1\6
...
\M{28}\B\X28:Hold-off all interrupts\X${}\E{}$\6
